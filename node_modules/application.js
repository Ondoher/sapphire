...
*/
	initialize : function(ns, req)
	{
		...
		this.masterFile = CONFIG.basePath + 'master.html';
	...
	makeExternalJavaScript : function()
	{
		...
		return this.getExternalFiles(javascriptFiles)
			.then(function(content)
			{
				...
				var preJavaScript = this.makeConfigJavaScript();
				javaScript = content;
				var postJavaScript = this.makePostJavaScript();
				...
				var javascript = javascriptCache.get(hash);
				if (javascript) return Q(preJavaScript + javascript + postJavaScript);
				...
				minify(options, function(error, data)
				{
					...
					else
					{
						...
						data = preJavaScript + data + postJavaScript;
